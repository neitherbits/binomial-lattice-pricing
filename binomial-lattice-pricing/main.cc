...
int main(int argc, char* argv[]) {
  model::Model m = model::Model<model::ModelStyle::JR_risk_neutral>{68.04,  0.0118, 0.0180,
                                                                    0.2433, 67.50,  173 / 365.0};
  ...
  recomb_tree::RecombinantTree r =
      recomb_tree::RecombinantTree<node::OptionStyle::European, node::OptionType::Put,
                                   model::ModelStyle::JR_risk_neutral>(m, periods);

  std::cout << "Model copied into RecombinantTree: (m.S) " << m.S << " == " << r.model_.S
            << " (r.model.S)" << std::endl;
  ...
  std::cout << "Final Node: period=" << n.period << " S=" << n.S << " value=" << n.value
            << std::endl;
